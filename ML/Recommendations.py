...
def sim_pearson(preferences, user1, user2):
    ...
    sum_sq_x1_y2 = sum(
        [preferences[user1][it] * preferences[user2][it] for it in si])

    n = len(si)

    num = (n * sum_sq_x1_y2) - ((x1_sum1 * y1_sum2))

    dem = sqrt((n * sumsq_x1) - (pow(x1_sum1, 2))) * \
        sqrt(((n * sumsq_y2) - (pow(y1_sum2, 2))))
...
def TopMatches(preferences, user, n=5, similiarity=sim_pearson):
    scores = [(similiarity(preferences, user, other), other)
              for other in preferences if other != user]
...
def getRecommendations(preferences, user, similiarity=sim_pearson):
    ...
    for other_users in preferences:
        if other_users == user:
            continue
        sim = similiarity(preferences, user, other_users)
...
def CalculateSimiliarItem(preferences, n=10):
    ...
    preferences = TransformPreference(preferences)
    for item in preferencesrev:
